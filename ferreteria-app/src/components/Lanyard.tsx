...
function Band({ maxSpeed = 50, minSpeed = 0 }: BandProps) {
  ...
  const meshNode = gltf.nodes.MagnetCard_MagnetCard_0;
  const meshMaterial =
    gltf.materials?.Material_0 || Object.values(gltf.materials || {})[0];

  if (!meshNode) console.warn("⚠️ No se encontró el nodo esperado en el GLB");
  if (!meshMaterial) console.warn("⚠️ No se encontró material en el GLB");
  ...
  useFrame((state, delta) => {
    ...
    if (
      ...
      if (band.current && (band.current as any).geometry?.setPoints) {
